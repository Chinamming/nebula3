...
D3D9MemoryVertexBufferLoader::OnLoadRequested()
{
    ...
    switch (this->usage)
    {
        case VertexBuffer::UsageDefault:
            d3dPool = D3DPOOL_SYSTEMMEM;
            break;

        case VertexBuffer::UsageImmutable:
            n_assert(VertexBuffer::AccessNone == this->access);
            d3dUsage |= D3DUSAGE_WRITEONLY;
            d3dPool   = D3DPOOL_MANAGED;
            break;

        case VertexBuffer::UsageDynamic:
            n_assert(VertexBuffer::AccessWrite == this->access);
            d3dUsage |= D3DUSAGE_WRITEONLY;
            d3dPool   = D3DPOOL_DEFAULT;
            break;

        default:
            n_error("D3D9MemoryVertexBufferLoader: unsupported or unknown Usage flag!");
            break;
    }
