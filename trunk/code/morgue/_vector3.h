#pragma once
#ifndef MATH__VECTOR3_H
#define MATH__VECTOR3_H
//------------------------------------------------------------------------------
/**
    @class Math::vector3
...
public:
    ...
    bool operator ==(const _vector3& v0) const;
    ...
    /// returns a new vector which is the lerp-result between v0 and v1
    static _vector3 lerp(const _vector3& v0, const _vector3& v1, float l);
...
inline
void
_vector3::clamp(float min, float max)
{
    x = n_clamp(x, min, max);
    y = n_clamp(y, min, max);
    z = n_clamp(z, min, max);
...
inline
_vector3
_vector3::lerp(const _vector3& v0, const _vector3& v1, float l)
{
    return v0 + ((v1 - v0) * l);
}
