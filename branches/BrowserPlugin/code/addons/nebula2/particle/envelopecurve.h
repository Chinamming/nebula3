#pragma once
#ifndef MATH_ENVELOPECURVE_H
#define MATH_ENVELOPECURVE_H
//------------------------------------------------------------------------------
/**
    @class Math::EnvelopeCurve
...
public:
    ...
    EnvelopeCurve();
    /// constructor 2
    EnvelopeCurve(const float keyFrameValue0, const float keyFrameValue1,
    ...
    void SetParameters(const EnvelopeCurve& src);
...
*/
inline
EnvelopeCurve::EnvelopeCurve(const float keyFrameValue0, 
...
*/
inline
void EnvelopeCurve::SetParameters(const float keyFrameValue0, const float keyFrameValue1,
    const float keyFrameValue2, const float keyFrameValue3,
    const float keyFramePos1, const float keyFramePos2,
    const float frequency, const float amplitude,
...
*/
inline
void EnvelopeCurve::SetParameters(const EnvelopeCurve& src)
{
...
*/
inline
float EnvelopeCurve::GetValue(float pos) const
{   
	pos = Math::n_saturate(pos);
    ...
    if (this->amplitude > 0.0f)
    {
        ...
        {
			value += Math::n_sin(pos * N_PI * 2 * this->frequency) * this->amplitude;
        }
        else
        {
			value += Math::n_cos(pos * N_PI * 2 * this->frequency) * this->amplitude;
        }
    }
...
*/
inline
float EnvelopeCurve::GetMaxPossibleValue() const
{
    float maxVal;
    int keyFrame;
