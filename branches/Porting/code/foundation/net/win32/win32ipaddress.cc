...
/**
*/
Win32IpAddress::Win32IpAddress(const String& hostName, ushort portNumber)
{
    Memory::Clear(&this->addr, sizeof(this->addr));
    this->addr.sin_family = AF_INET;
...
{
    this->addr = sa;
    this->hostName = inet_ntoa(sa.sin_addr);
...
Win32IpAddress::SetHostName(const String& n)
{
    n_assert(n.IsValid());
    this->hostName = n;
    if (!this->GetHostByName(n, this->addr.sin_addr))
    {
        
    }
    this->addrAsString = inet_ntoa(this->addr.sin_addr);
}
