...
{   
    ...
    NPPluginBase* plugin = NS_NewPluginInstance(&ds);
    if (!plugin)
...
NPError NPP_Destroy(NPP instance, NPSavedData** save)
{
    ...
    NPPluginBase* plugin = (NPPluginBase *)instance->pdata;
    if (plugin) 
    {
        plugin->Close();
...
{    
    ...
    // window just created
    if (!plugin->IsOpen() && pNPWindow->window)
    {
        if (!plugin->Open(pNPWindow)) 
        {
            NS_DestroyPluginInstance(plugin);
            instance->pdata = 0;
    ...
    // window goes away
    if ((!pNPWindow->window) && plugin->IsOpen())
