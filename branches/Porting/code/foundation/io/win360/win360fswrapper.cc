...
/**
    ...
    to the file which must be passed to the other Xbox360FSWrapper file methods.
...
Win360FSWrapper::GetFileWriteTime(const String& path)
{
    ...
    {
        n_error("Xbox360FSWrapper::GetFileWriteTime(): failed to open file '%s'!", path.AsCharPtr());
    }
    return fileTime;
}
