...
Entity::AttachProperty(const Ptr<Property>& prop)
{
    ...
    if (!this->propertyProfilersByClass.Contains(prop->GetRtti()))
    {
        Util::String name = Util::String("profManga.Frame.Game.OnFrame.") + EntityManager::RTTI.GetName() +
            "." + this->GetRtti()->GetName() + "." + prop->GetRtti()->GetName() + " ";
        nProfiler profiler;
        profiler.Initialize(name.Get());
        
        this->propertyProfilersByClass.Add(prop->GetRtti(), profiler);
    }
    #endif
}
