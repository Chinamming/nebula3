...
CmdLineArgsTest::Run()
{
    ...
    const int argc = 12;
    CmdLineArgs posixArgs(argc, argv);

    // create Win32 style cmd line args object
    CmdLineArgs winArgs("mycommand -help -file filename.txt -retries 10 -weight 1.34 -pos \"1.03, 4.53, 10.2 1.0\" -color \"1.0,2.0,3.0,1.0\"");
    ...
    this->Verify(posixArgs.GetBool("-help") == true);
    ...
    this->Verify(winArgs.GetBool("-help") == true);
